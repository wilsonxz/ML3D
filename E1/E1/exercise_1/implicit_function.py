...
def signed_distance_sphere(x, y, z, r, x_0, y_0, z_0):
    ...
    f = np.sqrt((x - x_0)**2 + (y - y_0)**2 + (z - z_0)**2) - r # signed distance
    return f
    # ###############
...
def signed_distance_atom(x, y, z):
    ...
    # ###############
    # TODO: Implement
    f_proton = signed_distance_sphere(x, y, z, proton_radius, proton_center[0], proton_center[1], proton_center[2])
    f_electron = signed_distance_sphere(x, y, z, electron_radius, electron_center[0], electron_center[1], electron_center[2])
    f_orbit= signed_distance_torus(x, y, z, orbit_radius, orbit_thickness, proton_center[0], proton_center[1], proton_center[2])
    return np.min(np.array([f_proton, f_electron, f_orbit]), axis=0) #axis=0 find min in row-direction downwards direction;axis=1 find min in     column-direction right direction
    # ###############
